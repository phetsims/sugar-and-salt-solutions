    ...
    randomWalk: function( particle, dt ) {
      ...
      if ( particle.hasSubmerged() && this.waterVolume.get() > 0 ) {
        ...
        if ( particle.velocity.get().magnitude() === 0 ) {
          particle.velocity.set( Vector2.createPolar( 1, RandomUtil.randomAngle() ) );
        }
        particle.velocity.set( particle.velocity.get().withMagnitude( SugarAndSaltConstants.FREE_PARTICLE_SPEED ) );
      }
      ...
      particle.stepInTime( this.model.getExternalForce( this.model.isAnyPartUnderwater( particle ) ).times(
        1.0 / SugarAndSaltConstants.PARTICLE_MASS ), dt );
      ...
      if ( underwater ) {
        var theta = Math.random() * Math.toRadians( 30.0 ) * RandomUtil.nextRandomSign();
        particle.velocity.set( particle.velocity.get().rotated( theta ) );
      }
      ...
      if ( initiallyUnderwater && !underwater ) {
        ...
        particle.velocity.set( Vector2.createPolar( initialVelocity.magnitude(), RandomUtil.randomAngle() ) );
      }
